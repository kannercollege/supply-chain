...
try:
    while lora.has_joined():
        ...
        order= ""
        while order == "":          
            order = s.recv(64).decode()
            time.sleep(0.3)
        
        s.setblocking(True)

        pr.print_on_receipt(order)

        correct_scan = False

        print('scanning for a video')
        while correct_scan == False:            
            ...
            if (atqa != 0):        
                ...
                if nfc.format_block(uid, uid_len) != "":
                    ...
                    # requesting title
                    print('sending sms to costumer')                    
                    s.send(uid)
                    s.setblocking(False) 
                    print("")                 

                    #lk.print_on_matrix("send: " + order)
